...
MERGE (node:`_code`:`_code_faster-indicators-metric` { value:"record-type" });
MATCH (parent:`_code_list`:`_code_list_faster-indicators-metric`),                (node:`_code`:`_code_faster-indicators-metric` { value:"record-type" })                 MERGE (node)-[:usedBy { label:"Record type"}]->(parent);
